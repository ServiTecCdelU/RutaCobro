...
- **Punitorios por mora** (`src/utils/punitorios.js`): recargo automático sobre cuotas vencidas — tasa diaria % sobre el saldo, con días de gracia y tope (% del monto de la cuota). Config en `config/negocio.punitorio` (`{ activo, tasaDiariaPct, diasGracia, topePct }`), editable por admin en `/configuracion` (`CardPunitorios`). Se muestra en Cobranza y en el detalle, y **al cobrar la cuota se registra como movimiento `tipo: 'punitorio'`** en la misma transacción (`cobrarCuota` acepta `{ punitorio }`; lo calcula `useCobrar`). Entra al cierre de caja; al revertir la cuota se elimina junto con el cobro.
- **Contrato/pagaré PDF** (`src/utils/contrato.js`): mutuo con cláusulas, monto en letras (`montoEnLetras`), cronograma y firmas; refleja la config de punitorios. Botón "Contrato" en `ModalDetalle` (descarga directa).
- **Auditoría** (colección `auditoria`, inmutable, solo lectura admin): `services.js` registra acciones sensibles (revertir-cuota, eliminar-prestamo, eliminar-cliente, refinanciar) con autor. El autor (`setAutorActual`, seteado por `AuthContext`) también se estampa en cada movimiento (`autorUid`/`autorEmail`, visible en Movimientos). Vista "Actividad sensible" al pie de `/equipo` (`ActividadReciente`).
