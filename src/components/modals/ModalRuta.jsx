...
const COLORES = ['#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#3b82f6', '#ec4899', '#14b8a6', '#f97316'];
...
export default function ModalRuta({ onClose, ruta }) {
  ...
  const [form, setForm] = useState(() => ruta
    ? { nombre: ruta.nombre ?? '', cobrador: ruta.cobrador ?? '', color: ruta.color ?? COLORES[0] }
  ...
  const handleGuardar = async () => {
    setError('');
    if (!form.nombre.trim()) { setError('Ingresá el nombre de la ruta'); return; }
    if (!form.cobrador.trim()) { setError('Ingresá el cobrador'); return; }
    setLoading(true);
    try {
      if (esEdicion) {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-[fadein_0.15s_ease-out]">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-md shadow-popover animate-[pop_0.18s_ease-out]">
        <div className="p-5 border-b border-slate-200 flex items-center justify-between">
          <h3 className="font-bold text-slate-900 text-lg">{esEdicion ? 'Editar ruta' : 'Nueva ruta'}</h3>
        ...
        <div className="p-5 space-y-4">
          {[['Nombre de la ruta', 'nombre', 'Ej: Centro'], ['Nombre del cobrador', 'cobrador', 'Ej: Juan Pérez']].map(([label, key, ph]) => (
            <div key={key}>
