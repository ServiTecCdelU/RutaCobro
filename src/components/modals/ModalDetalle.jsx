...
import { X, CheckCircle2, Undo2, Wallet, Banknote } from 'lucide-react';
...
export default function ModalDetalle({ prestamoId, onClose }) {
  ...
  const handleCobrar = async (nro) => {
    try {
      const res = await cobrarCuota(prestamo.id, nro);
      toast.success(
        res.finalizado ? '¡Préstamo finalizado!' : 'Cuota cobrada',
        {
          description: `Cuota ${nro}/${res.cuotasTotales} · ${formatMoney(res.monto)}`,
          action: {
            label: 'Deshacer',
            onClick: async () => {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-[fadein_0.15s_ease-out]">
        <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-2xl max-h-[92vh] flex flex-col shadow-popover animate-[pop_0.18s_ease-out]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {cliente?.dni ? `DNI ${cliente.dni} · ` : ''}{ruta?.nombre ?? 'Sin ruta'}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3 p-5 bg-gradient-to-br from-slate-50 to-brand-50/30 flex-shrink-0 border-b border-slate-100">
            {[
              ...
              <div key={label}>
                ...
                <div className={`text-base sm:text-lg font-bold tabular-nums ${color} truncate`}>{value}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 overflow-auto p-5">
            <div className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Cronograma</div>
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                      c.pagada ? 'bg-emerald-500 text-white'
                        : parcial ? 'bg-amber-500 text-white'
                        : atraso > 0 ? 'bg-rose-500 text-white'
                    ...
                    <div className="text-sm font-bold text-slate-900 flex-shrink-0 tabular-nums">{formatMoney(c.monto)}</div>
                    {c.pagada ? (
                      <button
                        onClick={() => setConfirmRevertir(c.nro)}
                        className="flex-shrink-0 px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 text-xs font-semibold hover:bg-slate-50 hover:text-slate-900 transition-colors inline-flex items-center gap-1"
      ...
      {mostrarPago && (
        <ModalPago prestamoId={prestamoId} onClose={() => setMostrarPago(false)} />
      )}
    </>
  );
}
