...
export default function ModalNuevoCliente({ onClose, cliente }) {
  ...
  const handleGuardar = async () => {
    ...
    if (!form.rutaId) { setError('Seleccioná una ruta'); return; }
    setLoading(true);
    try {
      if (esEdicion) {
        ...
        {rutas.length === 0 ? (
          <div className="p-5">
            <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-900">Necesitás crear una ruta primero</p>
                <p className="text-xs text-amber-700 mt-1">Los clientes tienen que estar asignados a una ruta de cobro.</p>
              </div>
            </div>
          ...
          <>
            <div className="p-5 space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Nombre completo</label>
                <input
                  type="text"
                  value={form.nombre}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">DNI</label>
                  <input
                    type="text"
                    value={form.dni}
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Teléfono</label>
                  <input
                    type="tel"
                    value={form.tel}
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Dirección</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Ruta</label>
                <select
                  value={form.rutaId}
                ...
                >
                  {rutas.map(r => (
                    <option key={r.id} value={r.id}>
                      {r.nombre}{r.cobrador ? ` · ${r.cobrador}` : ''}
                    </option>
                  ))}
                </select>
              </div>
