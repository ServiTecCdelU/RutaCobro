...
export default function MetricCard({ label, value, change, icon: Icon, accent, sublabel }) {
  return (
    <div className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-white border border-slate-200/70 p-3 sm:p-5 hover:border-slate-300 hover:shadow-card-hover transition-all duration-200 group shadow-card">
      <div
      ...
      <div className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5 sm:mb-1 relative truncate">
        {label}
      </div>
      <div className="text-lg sm:text-2xl font-bold text-slate-900 tracking-tight truncate relative">
        {value}
      </div>
      {sublabel && (
        <div className="text-[11px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 truncate relative">
          {sublabel}
        </div>
      )}
    </div>
