...
function Cotiz({ nombre, dato, acento }) {
  return (
    <div className="min-w-0">
      <div className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
        {nombre}
      </div>
      <div className="text-base sm:text-xl font-bold text-slate-900 dark:text-slate-100 tabular-nums truncate">
...
export default function CotizacionDolar({ pesos }) {
  ...
  if (cargando && !dolares) {
    return (
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/70 p-5 shadow-card">
    ...
    return (
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/70 p-5 shadow-card">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            onClick={refrescar}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors flex-shrink-0"
          >
    ...
    <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/70 p-5 shadow-card">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center flex-shrink-0">
            <DollarSign size={18} />
          </div>
          <div className="min-w-0">
        ...
        <button
          ...
          className="w-9 h-9 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors flex-shrink-0"
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-[11px] text-slate-500 dark:text-slate-400 truncate">al blue</span>
          </div>
        </div>
      </div>
    </div>
