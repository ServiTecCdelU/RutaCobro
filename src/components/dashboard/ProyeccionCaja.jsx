      ...
      <div className="flex items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="font-bold text-slate-900">Proyección de cobranza</h2>
            <p className="text-xs text-slate-500">Próximas 4 semanas según vencimientos</p>
          </div>
        </div>
        ...
        {filas.map((f) => (
          <div key={f.label} className="flex items-center gap-3">
            <div className="w-32 sm:w-40 flex-shrink-0">
              <div
                className={`text-xs font-semibold ${f.esVencido ? 'text-rose-600' : 'text-slate-700'}`}
              >
                {f.label}
              </div>
              {f.detalle && <div className="text-[10px] text-slate-400">{f.detalle}</div>}
            </div>
