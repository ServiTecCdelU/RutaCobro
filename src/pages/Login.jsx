  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modo === 'login') {
        ...
        setSuccess(`Enviamos un mail a ${email}. Revisá tu bandeja (y spam) para restablecer la contraseña.`);
      }
    } catch (err) {
    ...
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-slate-50">
      ...
      <div className="relative w-full max-w-sm animate-[fadein_0.4s_ease-out]">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900 font-display tracking-tight">RutaCobro</h1>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-white/60 ring-1 ring-slate-900/5 p-6 shadow-elevated">
          <div className="flex items-center gap-2 mb-5">
            {modo === 'reset' && (
              <button
                onClick={() => { setModo('login'); resetFeedback(); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Email</label>
              <input
                type="email"
                value={email}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {modo === 'login' && (
                    <button
                      type="button"
                      onClick={() => { setModo('reset'); resetFeedback(); }}
          ...
          {modo !== 'reset' && (
            <>
              <div className="flex items-center gap-3 my-5">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">o</span>
                <div className="flex-1 h-px bg-slate-200" />
              </div>
          ...
          {modo !== 'reset' && (
            <div className="mt-5 text-center text-sm text-slate-500">
              ...
              <button
                onClick={() => { setModo(m => m === 'login' ? 'registro' : 'login'); resetFeedback(); }}
