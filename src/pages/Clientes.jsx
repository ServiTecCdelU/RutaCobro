...
export default function Clientes() {
  const {
    clientes, prestamos, rutas,
    cobrarCuota, revertirCuota, eliminarCliente, eliminarPrestamo,
    error,
  } = useApp();
  ...
  const handleCobrar = async (prestamoId) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight font-display">Clientes</h1>
          <p className="text-sm text-slate-500 mt-1 tabular-nums">
            {clientes.length} {clientes.length === 1 ? 'cliente' : 'clientes'} · {prestamos.length} {prestamos.length === 1 ? 'préstamo' : 'préstamos'}
          </p>
        </div>
        <button
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-semibold transition-colors ${
                showFiltros ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-700 border-slate-200 hover:border-slate-300'
              }`}
            >
          ...
          {showFiltros && (
            <div className="flex flex-col gap-3 p-4 bg-white rounded-2xl border border-slate-200/70 shadow-card">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Estado</p>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
                        filtroEstado === val ? 'bg-brand-600 text-white shadow-brand-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >
                      {label}
                    </button>
          ...
          {items.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {items.map(({ cliente, prestamo }) => {
                const ruta = rutas.find(r => r.id === cliente.rutaId);
                if (!prestamo) {
                  return (
                    <div key={`sin-${cliente.id}`} className="relative bg-white rounded-2xl border border-slate-200/70 p-4 pl-5 shadow-card hover:shadow-card-hover transition-all overflow-hidden">
                      <span
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <ActionMenu actions={[
                          { label: 'Editar cliente', icon: Pencil, onClick: () => setModalCliente({ editar: cliente }) },
                          { label: 'Eliminar cliente', icon: Trash2, danger: true, onClick: () => setConfirmBorrarCliente(cliente) },
                        ]} />
                      </div>
                    </div>
                  );
      ...
      {modalCliente?.nuevo && (
        <ModalNuevoCliente onClose={() => setModalCliente(null)} />
      )}
      {modalCliente?.editar && (
        <ModalNuevoCliente onClose={() => setModalCliente(null)} cliente={modalCliente.editar} />
      )}
      {modalPrestamo && (
        <ModalNuevoPrestamo onClose={() => setModalPrestamo(null)} clienteIdInicial={modalPrestamo} />
      )}
