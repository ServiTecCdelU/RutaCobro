...
export default function Equipo() {
  const {
    user, tenantId, esAdmin, rutasAll, error,
  ...
  const handleCrear = async () => {
    if (!emailNuevo.trim() || !rutaNueva) {
      toast.error('Completá email y ruta');
      return;
    }
    setCreando(true);
    try {
      const token = await crearInvitacion({ email: emailNuevo, rutaId: rutaNueva });
      setEmailNuevo('');
      setRutaNueva('');
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight font-display">Equipo</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/70 p-5 shadow-card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="sm:col-span-2">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Email del cobrador</label>
            <input
              type="email"
              value={emailNuevo}
          ...
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2 block">Ruta asignada</label>
            <select
              value={rutaNueva}
        ...
        <button
          onClick={handleCrear}
          disabled={creando || !emailNuevo.trim() || !rutaNueva}
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-card">
          ...
          <div className="space-y-2">
            {invitaciones.map(inv => {
              ...
              return (
                <div key={inv.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200">
                  ...
                  >
                    {copiado === inv.id ? <><Check size={13} /> Copiado</> : <><Copy size={13} /> Copiar link</>}
                  </button>
                  <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">{miembros.length} {miembros.length === 1 ? 'persona' : 'personas'}</p>
          </div>
        </div>
        {miembros.length === 0 ? (
                ...
                <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 truncate">
                      {m.email || m.id} {esYo && <span className="text-slate-400 font-normal">· vos</span>}
                    </p>
                    <p className="text-xs text-slate-500">
                      <span className="font-semibold">{esAdminMiembro ? 'Admin' : 'Cobrador'}</span>
                      {!esAdminMiembro && ruta && <> · Ruta: <span className="font-semibold">{ruta.nombre}</span></>}
                    </p>
                  </div>
                  {!esAdminMiembro && !esYo && (
                    <button
